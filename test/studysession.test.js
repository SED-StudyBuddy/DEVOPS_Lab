...
describe('Study Session API', () => {
  ...
  it('should filter study sessions by type (virtual)', async () => {
    ...
    expect(res.statusCode).toEqual(200)
    expect(res.body.every(s => s.type === 'virtual')).toBe(true)
  })
  ...
  it('should create a new study session', async () => {
    ...
    expect(res.body.participants).toContain(USER_ID)
  ...
  // ---------------------------------------------------------------------------
  // Test 8: Edit a study session
  // ---------------------------------------------------------------------------
  it('should edit an existing study session', async () => {
  ...
  it('should allow a student to join a public study session', async () => {
    const res = await request(app)
      .post('/api/study-sessions/102/join')
  ...
  it('should return 409 when trying to join a full session', async () => {
    const res = await request(app)
      .post('/api/study-sessions/102/join')
      .send({ userId: 'new_user@esilv.fr' })

    if (res.statusCode === 409) {
      expect(res.body).toHaveProperty(
        'message',
        'Cannot join session: Maximum capacity reached'
      )
    } else {
  ...
  it('should allow a student to leave a study session', async () => {
    const res = await request(app)
      .post('/api/study-sessions/102/leave')
      .send({ userId: OTHER_USER_ID })
